...
function OTPForm(){
  ...
  const email=params.get("email")||"yourname@gmail.com";
  ...
  const resend=async(e)=>{
    ...
    setResending(true);
    try{
      const r=await fetch("/api/otp/resend",{method:"POST"});
      const d=await r.json();
      if(!r.ok)throw new Error(d.message||"Could not resend OTP.");
      setSeconds(45);
    }catch(err){
      setError(err.message);
    }finally{
  ...
  return <>
    ...
    <div className="made">🇮🇳 MADE IN INDIA</div>
    <h1>Verify Your Gmail</h1>
    <p>Enter the 6-digit code sent to <b>{email}</b>.</p>
    <form onSubmit={verify}>
      <label>6-Digit OTP
        <input value={otp} onChange={e=>setOtp(e.target.value.replace(/\D/g,"").slice(0,6))} inputMode="numeric" maxLength="6" placeholder="123456" required/>
      </label>
